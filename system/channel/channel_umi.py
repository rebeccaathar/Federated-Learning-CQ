...
class Model(tf.keras.Model):
    ...
    def call(self, batch_size, ebno_db):        
        ...
        noise.append(no)
        datarate = calculate_data_rate(self._rg.bandwidth, ebno_db, self._num_bits_per_symbol, self._coderate)  
        # datarate = round(datarate, 4)
        data_rate.append(datarate)
...
SIMS = {
    "ebno_db" : list(np.arange(-5, 25, 1.25)),
    "scenario" : ["umi"],
    "perfect_csi" : [False],
    "ber" : [],
    "bler" : [],
...
for scenario in SIMS["scenario"]:
    for perfect_csi in SIMS["perfect_csi"]:
        ...
        ber, bler , bit_errors, block_errors, nb_bits, nb_blocks = sim_ber(model, 
                            SIMS["ebno_db"],
                            batch_size=128,
                            max_mc_iter=4,
        ...
        SIMS["bit erros"].append(list(bit_errors.numpy()))
        SIMS["block erros"].append(list(block_errors.numpy()))
        SIMS["nb bits"].append(list(nb_bits.numpy()))
        SIMS["nb blocks"].append(list(nb_blocks.numpy()))
 
SIMS["duration"] = time.time() -  start

noise_numpy = [n.numpy() for n in noise]
noise_numpy = noise_numpy[-(len(SIMS["ebno_db"])):]
# print(len(noise_numpy))
# print(len(SIMS["ber"][-1]))
# print(len(SIMS["ebno_db"]))
data_rate = data_rate[-(len(SIMS["ebno_db"])):]

# print(noise_numpy)
#Salvar os dados em um arquivo CSV
csv_filename = "channel_metrics.csv"

# Salvar os resultados em um arquivo CSV
with open(csv_filename, 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile)
    # Escrever cabeçalhos
    csvwriter.writerow(['ebno_db', "no" , "data rate", 'ber','bler','bit erros','block erros','nb bits','nb blocks'])
    # Escrever os dados linha pr linha
    for i in range(len(SIMS["ber"][-1])):
        # print(i)
        row = [SIMS["ebno_db"][i], noise_numpy[i], data_rate[i],  SIMS["ber"][-1][i], 
               SIMS["bler"][-1][i], SIMS["bit erros"][-1][i], 
               SIMS["block erros"][-1][i], SIMS["nb bits"][-1][i], 
               SIMS["nb blocks"][-1][i]]
